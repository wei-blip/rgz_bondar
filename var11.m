...
%% Пункт 3
close all
% Из предыдущего пункта получили что Fв = 1.8
F_high = 1.8;
df = 0.0011;
f = df:df:F_high; 
f = [f F_high];
...
%% Пункт 4
close all
% Из предыдущего раздела получили что Fe = 0.9408
...
%% Пункт 5
% В предыдущих разделах получили что f_t_new = 3.0922, F_high = 1.8
close all
F_high = 1.8;
f_t_new = 3.0922;
T_t = 1/f_t_new;
df = 0.0011;
f = df:df:F_high*10*f_t_new;
Fd = f_t_new*100;
...
ylim([-60 0])
%% Пункт 6
% Из предыдущих разделов получили что Fe_needed = 2.9091
close all
Fe_needed = 2.9091;
q_dB = 0:20;
q = db2mag(q_dB);
